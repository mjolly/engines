...
engineOpen <- function(handle, md){
	...
	if(mpmode[handle] == "visual") {
		...
		file <- paste(stagingdir, "/", outputfile, "/", outputfile, ".png", sep ='');
		png(file);
	}
}
...
engineRead <- function(handle, records){
	...
	file <- paste(stagingdir, "/", inputfiledir, "/", inputfilename, ".data", sep ='');
	if(records == 0) {
...
engineWrite <- function(handle, data, mode) {
	...
	file <- paste(stagingdir, "/", outputfiledir, "/", outputfilename, ".data", sep ='');
	if(mpmode[handle] == "visual") {
		dev.off();
	} else if(mode == 'a') {
